var vidyard_player_width_No8gmngehgM2MtgVRHWgup = 640;
var vidyard_player_height_No8gmngehgM2MtgVRHWgup = 360;
var vidyard_html5_No8gmngehgM2MtgVRHWgup = false;
var vidyard_secure_No8gmngehgM2MtgVRHWgup = false;
var vidyard_integration_check_No8gmngehgM2MtgVRHWgup = function() {};
var vidyard_No8gmngehgM2MtgVRHWgup_params = {};
var vidyard_No8gmngehgM2MtgVRHWgup_raw_params = '';
...
(function() {
  ...
  var embed_script = document.getElementById('vidyard_embed_code_No8gmngehgM2MtgVRHWgup');
  var playerDetails = {"keywords":"","thumbnailUrl":"https://play.vidyard.com/No8gmngehgM2MtgVRHWgup.jpg","@context":"http://schema.org/","@type":"VideoObject","id":"No8gmngehgM2MtgVRHWgup","description":"See how disparate data in a tag-based architecture is simplified and given context with an object-based ecosystem.","name":"Smart Objects - Smart Data Animation","transcript":"","uploadDate":"2020-06-17T16:17:40.000Z","duration":"T0M20S","embedUrl":"https://play.vidyard.com/No8gmngehgM2MtgVRHWgup","height":360,"width":640};
  ...
  function check_for_duplicate_embeds() {
    ...
    for (var i = 0; i < all_scripts.length; ++i) {
      if (all_scripts[i].getAttribute('id') === "vidyard_embed_code_No8gmngehgM2MtgVRHWgup") {
    ...
    if (dupe_scripts.length >= 1) {
      ...
      if (typeof document.querySelectorAll === 'function') {
        ...
        for (var i = 0; i < wrappers.length; ++i) {
          if (wrappers[i].getAttributeNode('onclick').value === "fn_vidyard_No8gmngehgM2MtgVRHWgup();") {
  ...
  if (!embed_script || !embed_script.src) {
    ...
    for (var i in scripts) {
      ...
      if (script.src && /^.+(vidyard\.com).*\/No8gmngehgM2MtgVRHWgup.*/.test(script.src)) {
        embed_script = script;
        break;
      }
    }
  ...
  if (embed_script && embed_script.src) {
    ...
    vidyard_No8gmngehgM2MtgVRHWgup_raw_params = rawQueryParams;
    ...
    embed_script.src.replace(/(?:[\?&])([^&=]*)=?([^&#]*)/g, function ($0, $1, $2) {
      if ($1) {
        switch ($1) {
          ...
          case 'vylegacy':
            vidyard_No8gmngehgM2MtgVRHWgup_params[$1] = (parseInt($2, 10) !== 0);
            break;
          case 'chapter':
            vidyard_No8gmngehgM2MtgVRHWgup_params[$1] = parseInt($2, 10);
            break;
          case 'width':
          ...
          case 'height':
            ...
            break;
          case 'X-VY-Signature':
            vidyard_secure_No8gmngehgM2MtgVRHWgup = true;
          default:
            vidyard_No8gmngehgM2MtgVRHWgup_params[$1] = $2;
            break;
        }
      }
    });
  ...
  if (!vidyard_No8gmngehgM2MtgVRHWgup_params.type || vidyard_No8gmngehgM2MtgVRHWgup_params.type !== 'lightbox') {
    vidyard_No8gmngehgM2MtgVRHWgup_params.type = 'inline';
  }

  // Determine Player Settings which effect embed
  var multi_chapter = false && !(typeof vidyard_No8gmngehgM2MtgVRHWgup_params.hide_html5_playlist !== 'undefined' && vidyard_No8gmngehgM2MtgVRHWgup_params.hide_html5_playlist);
  var playlist_always_open = false;
  if (typeof vidyard_No8gmngehgM2MtgVRHWgup_params.playlist_always_open !== 'undefined') {
    playlist_always_open = vidyard_No8gmngehgM2MtgVRHWgup_params.playlist_always_open;
  }
  ...
  if (query_player_width && query_player_width > 0 && query_player_height && query_player_height > 0) {
    vidyard_player_width_No8gmngehgM2MtgVRHWgup = query_player_width;
    vidyard_player_height_No8gmngehgM2MtgVRHWgup = query_player_height;
  } else if (query_player_width && query_player_width > 0) {
    query_player_height = (query_player_width * vidyard_player_height_No8gmngehgM2MtgVRHWgup) / vidyard_player_width_No8gmngehgM2MtgVRHWgup;
    vidyard_player_width_No8gmngehgM2MtgVRHWgup = query_player_width;
    vidyard_player_height_No8gmngehgM2MtgVRHWgup = query_player_height;
  } else if (query_player_height && query_player_height > 0) {
    query_player_width = (query_player_height * vidyard_player_width_No8gmngehgM2MtgVRHWgup) / vidyard_player_height_No8gmngehgM2MtgVRHWgup;
    vidyard_player_width_No8gmngehgM2MtgVRHWgup = query_player_width;
    vidyard_player_height_No8gmngehgM2MtgVRHWgup = query_player_height;
  }

  var iframe_container = document.getElementById("vidyard_No8gmngehgM2MtgVRHWgup");
  ...
  if (!iframe_container && vidyard_No8gmngehgM2MtgVRHWgup_params.type === 'inline') {
    iframe_container = document.createElement("span");
    iframe_container.id = 'vidyard_No8gmngehgM2MtgVRHWgup';
  ...
  if (vidyard_secure_No8gmngehgM2MtgVRHWgup === false) {
    // Get cookie data/ids from various MAPs/CRMs
    var pardot_id = fn_getCookie("pardot");
    if (pardot_id) {
      vidyard_No8gmngehgM2MtgVRHWgup_params.pardot_id = pardot_id;
    }

    var hubspot_id = fn_getCookie("hubspotutk");
    if (hubspot_id) {
      vidyard_No8gmngehgM2MtgVRHWgup_params.hubspot_id = hubspot_id;
    }
    ...
    if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
      vidyard_No8gmngehgM2MtgVRHWgup_params.marketo_id = encodeURIComponent(marketo_id);
    }

    var dreamforce_id = fn_getCookie("vy_dreamforce");
    if (dreamforce_id) {
      vidyard_No8gmngehgM2MtgVRHWgup_params.dreamforce_id = encodeURIComponent(dreamforce_id);
    }
    ...
    if (vycustom_id !== "") {
      vidyard_No8gmngehgM2MtgVRHWgup_params.custom_id = vycustom_id;
    }

    // Gets the vyemail query string param, which connects views to an email entered
    var vyemail = get_parameter_by_name('vyemail');
    if (vyemail !== "") {
      vidyard_No8gmngehgM2MtgVRHWgup_params.email = vyemail;
    }

    var vyac = get_parameter_by_name('vyac');
    var vyplayer = get_parameter_by_name('vyplayer');
    if (vyplayer === 'No8gmngehgM2MtgVRHWgup') {
      vidyard_No8gmngehgM2MtgVRHWgup_params.access_code = vyac;
    }
  }
  ...
  if (navigator.userAgent.toLowerCase().match(/ip(ad|od|hone)|android/)) {
    vidyard_html5_No8gmngehgM2MtgVRHWgup = true;
    vidyard_No8gmngehgM2MtgVRHWgup_params.disable_popouts = true;
  }
  ...
  function create_embed() {
    ...
    var integration_tracking_check = function() {
      if (playerReady) {
        ...
        window.addEventListener('message', function callback(e) {
          ...
          if (data.event === "ready" && data.uuid === "No8gmngehgM2MtgVRHWgup") {
    ...
    var integration_tracking_check_original = function () {
      ...
      if (!referringUrlSent && vidyard_secure_No8gmngehgM2MtgVRHWgup === true && vidyard_No8gmngehgM2MtgVRHWgup_params.referring_url) {
        var playerFrame = Vidyard.Helpers.getPlayerIFrame('No8gmngehgM2MtgVRHWgup');
        var message = {
          'event': 'parentReferer',
          'uuid': 'No8gmngehgM2MtgVRHWgup',
          'data': {
            'value': vidyard_No8gmngehgM2MtgVRHWgup_params.referring_url
          }
        }
      ...
      if (!vidyard_No8gmngehgM2MtgVRHWgup_params.pardot_id) {
        var pardot_id = fn_getCookie("pardot");
        if (pardot_id) {
          vidyard_No8gmngehgM2MtgVRHWgup_params.pardot_id = pardot_id;
      ...
      if (!vidyard_No8gmngehgM2MtgVRHWgup_params.hubspot_id) {
        var hubspot_id = fn_getCookie("hubspotutk");
        if (hubspot_id) {
          vidyard_No8gmngehgM2MtgVRHWgup_params.hubspot_id = hubspot_id;
      ...
      if (!vidyard_No8gmngehgM2MtgVRHWgup_params.dreamforce_id) {
        var dreamforce_id = fn_getCookie("vy_dreamforce");
        if (dreamforce_id) {
          dreamforce_id = encodeURIComponent(dreamforce_id);
          vidyard_No8gmngehgM2MtgVRHWgup_params.dreamforce_id = dreamforce_id;
          messages.push({'data': {'type': 'dreamforce', 'value': dreamforce_id}});
        }
      }
      if (!vidyard_No8gmngehgM2MtgVRHWgup_params.marketo_id) {
        var marketo_id = fn_getCookie("_mkto_trk");
        var munchkin_id = '';
        
        if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
          marketo_id = encodeURIComponent(marketo_id);
          vidyard_No8gmngehgM2MtgVRHWgup_params.marketo_id = marketo_id;
        ...
        if (!eloquaSent &&
            vidyard_No8gmngehgM2MtgVRHWgup_params.eloqua_id &&
            ((vidyard_secure_No8gmngehgM2MtgVRHWgup === true && vidyard_No8gmngehgM2MtgVRHWgup_params.type === 'lightbox') ||
             (vidyard_No8gmngehgM2MtgVRHWgup_params.type === 'inline'))) {
          eloquaSent = true;
          messages.push({'data': {'type': 'eloqua', 'value': vidyard_No8gmngehgM2MtgVRHWgup_params.eloqua_id}});
        }
      

      // inline embeds are ready to receive api requests
      // lightbox will pass data through querystring params
      if (vidyard_No8gmngehgM2MtgVRHWgup_params.type === "inline" || vidyard_secure_No8gmngehgM2MtgVRHWgup === true) {
        for(var i in messages) {
          messages[i].event = 'associateVisitor';
          messages[i].uuid = 'No8gmngehgM2MtgVRHWgup';
          Vidyard.Helpers.getPlayerIFrame('No8gmngehgM2MtgVRHWgup').contentWindow.postMessage(JSON.stringify(messages[i]), window.location.protocol + '//play.vidyard.com');
        }
      }
    ...
    vidyard_integration_check_No8gmngehgM2MtgVRHWgup = integration_tracking_check;

    // Display the inline or lightbox
    if (vidyard_No8gmngehgM2MtgVRHWgup_params.type === "inline") {
      // Load the iframe
      iframe_container.innerHTML = fn_vidyard_build_iframe_No8gmngehgM2MtgVRHWgup(false, vidyard_player_width_No8gmngehgM2MtgVRHWgup, vidyard_player_height_No8gmngehgM2MtgVRHWgup);

      Vidyard.Helpers.addListener(
        'load',
        'onload',
        function() {
          fn_vidyard_iframe_after_load_No8gmngehgM2MtgVRHWgup();
          integration_tracking_check();
        },
        Vidyard.Helpers.getPlayerIFrame('No8gmngehgM2MtgVRHWgup')
      );

    } else if (vidyard_No8gmngehgM2MtgVRHWgup_params.type === "lightbox") {
      ...
      if (document.readyState === 'complete') {
        if (!vidyard_secure_No8gmngehgM2MtgVRHWgup) {
          integration_tracking_check();
        }
    ...
    function getEloquaCookie() {
      if (timerId !== null) {
        ...
        if (typeof this.GetElqCustomerGUID === 'function') {
          ...
          vidyard_No8gmngehgM2MtgVRHWgup_params.eloqua_id = eloqua_id;
          return;
        } else if (this._elqQ) {
  ...
  Vidyard._players['No8gmngehgM2MtgVRHWgup'] = '';
}());
...
var vyContactCallback = function(data) {
  ...
  vidyard_No8gmngehgM2MtgVRHWgup_params.marketo_lead = marketoData;
  ...
  // or tried again after IFrame loads in fn_vidyard_iframe_after_load_No8gmngehgM2MtgVRHWgup
  playerIframe = Vidyard.Helpers.getPlayerIFrame('No8gmngehgM2MtgVRHWgup');
  if ( playerIframe ) {
    message = {'data': {'type': 'marketo', 'value': marketoData}};
    message.event = 'identifyVisitor';
    message.uuid = 'No8gmngehgM2MtgVRHWgup';
...
(function () {
  function async_load() {
    var contactId = vidyard_No8gmngehgM2MtgVRHWgup_params.marketo_id;
    if (!contactId) {
    ...
    var url = window.location.protocol + '//play.vidyard.com'
        + '/v3/No8gmngehgM2MtgVRHWgup/contact/' + encodeURIComponent(contactId) + '/marketoContact.js';
...
function fn_vidyard_build_iframe_No8gmngehgM2MtgVRHWgup(lightbox, iframe_width, iframe_height) {
  if (lightbox) {
    vidyard_No8gmngehgM2MtgVRHWgup_params.autoplay = true;
  }

  var referring_url = encodeURIComponent(document.referrer);
  if (referring_url) {
    vidyard_No8gmngehgM2MtgVRHWgup_params.referring_url = referring_url;
  }
  ...
  if (document.documentMode === 7) {
    ...
    var ie7RepaintIframe = function() {
      var iframe = Vidyard.Helpers.getPlayerIFrame('No8gmngehgM2MtgVRHWgup');
  ...
  if (vidyard_secure_No8gmngehgM2MtgVRHWgup === true) {
    query_params_string = vidyard_No8gmngehgM2MtgVRHWgup_raw_params;
  } else {
    for (var param in vidyard_No8gmngehgM2MtgVRHWgup_params) {
      var param_value = vidyard_No8gmngehgM2MtgVRHWgup_params[param];
      if (param_value === true) {
        param_value = '1';
      }
  ...
  return '<span id="vidyard_span_No8gmngehgM2MtgVRHWgup" style="display: block; margin: auto; position: relative; width: ' + iframe_width + 'px; height: ' + iframe_height + 'px;"><iframe id="vidyard_iframe_No8gmngehgM2MtgVRHWgup" class="vidyard_iframe" src="//play.vidyard.com/No8gmngehgM2MtgVRHWgup?' + query_params_string + '" width="' + iframe_resizer + '" height="100%" title="Video" aria-label="Video" scrolling=no frameborder=0 allowtransparency="true" allowfullscreen allow="autoplay" style="opacity: 0; background-color: transparent; position: absolute; ' + ios_width_fix + ' right: 0px; top: 0px;"></iframe></span>';
}

// --- If the browser is FF, reload the iframe with the given id
function fn_vidyard_iframe_after_load_No8gmngehgM2MtgVRHWgup() {
  var _theframe = Vidyard.Helpers.getPlayerIFrame('No8gmngehgM2MtgVRHWgup');
  if (_theframe) {
    ...
    if (vidyard_secure_No8gmngehgM2MtgVRHWgup === true && vidyard_No8gmngehgM2MtgVRHWgup_params.type === 'lightbox') {
      vidyard_integration_check_No8gmngehgM2MtgVRHWgup();
    }
    ...
    if (vidyard_No8gmngehgM2MtgVRHWgup_params.marketo_lead) {
      var message = {'data': {'type': 'marketo', 'value': vidyard_No8gmngehgM2MtgVRHWgup_params.marketo_lead }};
      message.event = 'identifyVisitor';
      message.uuid = 'No8gmngehgM2MtgVRHWgup';
      Vidyard.Helpers.getPlayerIFrame('No8gmngehgM2MtgVRHWgup')
...
function fn_vidyard_No8gmngehgM2MtgVRHWgup() {
  var vidyard = {};
    ...
    return {
      show:function(initial_iframe_settings) {
        var showCore = function () {
          ...
          var width_height = this.lock_to_aspect_ratio(iframe_settings.width, iframe_settings.height, (319 + (vidyard_player_width_No8gmngehgM2MtgVRHWgup / 5)));
      ...
      fill_iframe: function(settings_html_url, has_element, use_post_request, should_animate, width, height) {
        if(has_element) {
          if(iframe_settings.image) {
            ...
            this.set_tinner_size(fn_vidyard_build_iframe_No8gmngehgM2MtgVRHWgup(true, width, height), should_animate, width, height);
          } else {
      ...
      resize: function() {
        ...
        var width_height = v_box.lock_to_aspect_ratio(
            vidyard_player_width_No8gmngehgM2MtgVRHWgup,
            vidyard_player_height_No8gmngehgM2MtgVRHWgup,
            319 + (vidyard_player_width_No8gmngehgM2MtgVRHWgup / 5)
        );

        iframe_settings.width = width_height[0];
        iframe_settings.height = width_height[1];

        var vidyard_iframe = Vidyard.Helpers.getPlayerIFrame('No8gmngehgM2MtgVRHWgup');
        var vidyard_span = document.getElementById('vidyard_span_No8gmngehgM2MtgVRHWgup');
      ...
      fade_element: function(element_to_fade, fading_in, target_alpha) {
        ...
        if (old_opacity == target_alpha) {
          ...
          if (fading_in == -1) {
            ...
            Vidyard.Helpers.getPlayerIFrame('No8gmngehgM2MtgVRHWgup').src = '';
  ...
  vidyard.box.show({
    iframe: '#', width: vidyard_player_width_No8gmngehgM2MtgVRHWgup,
    height: vidyard_player_height_No8gmngehgM2MtgVRHWgup,
    animate: !vidyard_html5_No8gmngehgM2MtgVRHWgup,
    fixed: true
  });
}
